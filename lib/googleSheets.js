...
function readAdsetNameFromCookie() {
  if (typeof document === "undefined") return "";
  try {
    const cookieEntry = document.cookie
      .split("; ")
      .find((entry) => entry.startsWith(`${ADSET_NAME_STORAGE_KEY}=`));
    if (!cookieEntry) return "";
    const rawValue = cookieEntry.slice(ADSET_NAME_STORAGE_KEY.length + 1);
    return decodeURIComponent(rawValue || "").trim();
  } catch {}
  return "";
}

function writeAdsetNameToCookie(value) {
  if (typeof document === "undefined") return;
  try {
...
function readAdsetNameFromSearch(search = "") {
  if (!search) return "";
  try {
    ...
    for (const key of ADSET_QUERY_KEYS) {
      ...
      if (rawValue && rawValue.trim()) {
        return normalizeAdsetName(rawValue);
      }
    }
  } catch {}
  return "";
...
function readCachedAdsetName() {
  ...
  const fromCookie = readAdsetNameFromCookie();
...
function cacheAdsetName(value) {
  ...
  writeAdsetNameToCookie(normalized);
}
...
function resolveAdsetName(extra = {}) {
  ...
  if (typeof window !== "undefined") {
    const fromUrl = readAdsetNameFromSearch(window.location.search || "");
    if (fromUrl && fromUrl !== "none") {
      cacheAdsetName(fromUrl);
      return fromUrl;
    }
  }
