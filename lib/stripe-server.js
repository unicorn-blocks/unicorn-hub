...
// Standard require - Webpack will trace this and bundle dependencies (stripe, qs, side-channel, etc.)
// Cloudflare build (esbuild) will see this but ignore it because of 'external: ["stripe"]' config
let Stripe;
let loadError = null;
...
try {
    ...
    // This catches runtime errors if module is missing, which is good safety
...
async function createCheckoutSession(params) {
    if (!Stripe) {
        throw new Error(`Stripe module not available. Env: ${process.env.NODE_ENV}. Load Error: ${loadError?.message || 'Unknown'}. Stack: ${loadError?.stack}`);
    }
